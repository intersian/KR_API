...
def calculate_higher_bid_volume(quote, order_price):
    """주문가격보다 높은 매수잔량 합산"""
    total_volume = 0
    
    # 매수호가 1~5까지 확인
    for i in range(1, 6):
        bid_price = float(quote.get(f'bond_bidp{i}', '0'))  # 채권 매수호가1~5
        bid_volume = int(quote.get(f'bidp_rsqn{i}', '0'))   # 매수호가 잔량1~5
        
        # 주문가격보다 높은 호가의 잔량만 합산
    ...
    try:
        ...
        if target_orders:
                ...
                if order_price == bid_price:  # 가격이 정확히 일치하는 경우만 패스
                    ...
                    if MIN_PRICE <= new_price <= MAX_PRICE:
                        ...
                        if new_price >= ask_price:
                            ...
                            # 주문가격보다 높은 매수잔량 확인
                            higher_volume = calculate_higher_bid_volume(quote, order_price)
                            print(f"주문가격보다 높은 매수잔량: {higher_volume:,}")
                            
                            if higher_volume <= 1000:
                                print("주문가격보다 높은 매수잔량이 부족하여 정정하지 않습니다.")
                            else:
            ...
            try:
                ...
                print(f"매수1호가: {bid_price:,.2f}")
                print(f"매도1호가: {ask_price:,.2f}")
                print(f"주문 예정 가격: {new_price:,.2f}")
                print(f"목표 가격 범위: {MIN_PRICE:,.2f} ~ {MAX_PRICE:,.2f}")
                
                # 주문 가격 결정
                if new_price > MAX_PRICE:
                    print("매수1호가 + 1원이 목표 범위를 벗어나 MAX_PRICE로 주문을 실행합니다.")
                    order_price = MAX_PRICE
                elif MIN_PRICE <= new_price <= MAX_PRICE:
                    if new_price >= ask_price:
                        print("매수1호가 + 1원이 매도1호가 이상이어서 매수1호가로 주문을 실행합니다.")
                        order_price = bid_price
                    else:
                        print("매수1호가 + 1원으로 주문을 실행합니다.")
                        order_price = new_price
                else:
                    print("매수1호가 + 1원이 목표 범위 미만이어서 주문을 실행하지 않습니다.")
                    return
                
                # 신규 매수 주문
                result = await api.place_bond_order(
                    symbol=TARGET_SYMBOL,
                    quantity=TARGET_QUANTITY,
                    price=order_price
                )
                print(f"신규 매수 주문 완료 (주문번호: {result.get('odno', '-')})")
                print(f"주문수량: {TARGET_QUANTITY:,}")
                print(f"주문가격: {order_price:,.2f}")
                print("-" * 30)
                
            except Exception as e:
                print(f"신규 주문 실패: {str(e)}")
